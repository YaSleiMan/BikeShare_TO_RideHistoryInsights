...
def cost_savings_calculator():
    ...
    bikeshare_subscription_cost = 120*1.13  # Cost of yearly subscription to Toronto Bike Share + taxes

    equivalent_ttc_tickets = 0
    for i in range(len(numeric_durations)-1):
        if numeric_durations[i] < (numeric_durations[i+1] + max_connection_time*60*60):
